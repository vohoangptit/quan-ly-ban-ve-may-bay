...
void FreeTreePassenger(NODE_PASSENGER* root)
{
	if (root == NULL)
		return;
	FreeTreePassenger(root->pLeft);
	FreeTreePassenger(root->pRight);
	free(root);
}
...
void OutputPassengerOnFlightDetail(PASSENGER p, string noTicket, int stt, int locate)
{
	DeleteOldData(sizeof(keyDisplayPassengerOnFlight) / sizeof(string), locate);
	Gotoxy(xKeyDisplay[0] + 1, Y_DISPLAY + 3 + locate); cout << stt;
	Gotoxy(xKeyDisplay[1] + 1, Y_DISPLAY + 3 + locate); cout << noTicket;
	Gotoxy(xKeyDisplay[2] + 1, Y_DISPLAY + 3 + locate); cout << p.id;
	Gotoxy(xKeyDisplay[3] + 1, Y_DISPLAY + 3 + locate); cout << p.firstName<<" "<<p.lastName;
		...
		while (_kbhit())
		{
			...
			if (key == 0 || key == 224)
			{
				...
				else if (key == KEY_F3) //f3
				{
					...
					if (k->data.isBooked > 0)
					{
						Gotoxy(X_NOTIFY, Y_NOTIFY);
						cout << "Nguoi nay co du lieu ve. K xoa duoc";
						key = _getch();
						DeleteNotify();
						goto backMenu;
					}
